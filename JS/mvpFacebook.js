...
mvpFacebook = new function() {
	...
	this.getLoginStatus = function(getLogInStatusCallback) {
		...
		FB.getLoginStatus(function(response) {
			mvpFacebook._fbConnectionStatus = response.status;
			if (response.status === 'connected') {
				mvpFacebook._fbUserId = response.authResponse.userID;
				mvpFacebook._fbToken = response.authResponse.accessToken;
				mvpFacebook._expiresIn = response.authResponse.expiresIn;
				// positionner le token de l'utilisateur en cookie
				mvpFacebook._setAuthTokenCookie(mvpFacebook._fbToken);
				// var expire = response.authResponse.expiresIn;
				// var signedRequest = response.authResponse.signedRequest;
			} else /* if (response.status === 'not_authorized' || response.status === 'not_logged_in') */ {
				mvpFacebook._fbUserId = null;
				mvpFacebook._fbToken = null;
				mvpFacebook._expiresIn = null;
			}
			if (getLogInStatusCallback) {
				getLogInStatusCallback(mvpFacebook._fbConnectionStatus, mvpFacebook._fbUserId, mvpFacebook._fbToken, mvpFacebook._expiresIn);
			}
		}, this._appScope);
	};
	/**
	...
	this.login = function(callbackFunction) {
		FB.login(function(response) {
			...
			mvpFacebook._setAppSubmittedCookie();
			if (callbackFunction) {
				callbackFunction(mvpFacebook._fbConnectionStatus, mvpFacebook._fbUserId, mvpFacebook._fbToken, mvpFacebook._expiresIn);
			}
		}, this._appScope);
	};
	...
	this._initFbSdk = function(initFbSdkCallback) {
		...
		js.onload = function() {
			FB.init({
				appId:		mvpFacebook._appId,	// App ID
		//		channelUrl:	'/fbChannel.html',	// Channel File
			...
			mvpFacebook.getLoginStatus(initFbSdkCallback);
		};
